...
function createSunBurst(svgId, data) {
  const arc = d3
    .arc()
    .startAngle((d) => d.x0)
    .endAngle((d) => d.x1)
  ...
  const label = g
    ...
    .text((d) => {
      return d.value > 1 ? d.data.name + ' (' + d.value + ')' : d.data.name;
    });
...
const taxonomyData = TAXONOMY_DATA;

// Create Instances
createSunBurst('#svg-file', fileData);
createSunBurst('#svg-taxonomy', taxonomyData);
